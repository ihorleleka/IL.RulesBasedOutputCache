...
internal sealed class RulesBasedOutputCacheStream : Stream
{
    ...
    internal RulesBasedOutputCacheStream(Stream innerStream, long maxBufferSize, int segmentSize, Action startResponseCallback)
    {
        _innerStream = innerStream;
        _maxBufferSize = maxBufferSize;
        _startResponseCallback = startResponseCallback;
        _segmentWriteStream = new SegmentWriteStream(segmentSize);
    }
    ...
    internal RulesBasedCacheCachedResponseBody GetCachedResponseBody()
    {
        if (!BufferingEnabled)
        {
            throw new InvalidOperationException("Buffer stream cannot be retrieved since buffering is disabled.");
        }
        return new RulesBasedCacheCachedResponseBody(_segmentWriteStream.GetSegments(), _segmentWriteStream.Length);
    }
