...
/// </summary>
internal static class RulesBasedOutputCacheEntrySerializer
{
    private const byte SerializationRevision = 1;

    public static async ValueTask<RulesBasedOutputCacheEntry?> GetAsync(string key, IOutputCacheStore store, CancellationToken cancellationToken)
    {
        ...
        var outputCacheEntry = new RulesBasedOutputCacheEntry
        {
    ...
    public static async ValueTask StoreAsync(string key, RulesBasedOutputCacheEntry value, TimeSpan duration, IOutputCacheStore store, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(value);
        ArgumentNullException.ThrowIfNull(value.Body);
        ArgumentNullException.ThrowIfNull(value.Headers);

        var formatterEntry = new SerializableOutputCacheEntry
        {
    ...
    private static void Serialize(Stream output, SerializableOutputCacheEntry entry)
    {
    ...
    private static SerializableOutputCacheEntry? Deserialize(Stream content)
    {
        ...
        var result = new SerializableOutputCacheEntry();
